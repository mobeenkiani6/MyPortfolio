    ...
    <section id="experience" className="py-20 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="absolute left-6 md:left-1/2 transform -translate-x-1/2 w-1 h-full bg-slate-800 rounded-full overflow-hidden">
            <motion.div
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                <div key={index} className="relative flex items-center justify-between w-full">
                  {/* Left Column */}
                  <div className={`w-full md:w-5/12 ${isEven ? 'md:pr-8 text-left md:text-right pl-16 md:pl-0' : 'order-1 hidden md:block opacity-0'}`}>
                    {isEven && (
                      <motion.div
                      ...
                      >
                        {isFirst && (
                          <div className="absolute -top-3 right-4 md:-right-3 bg-violet-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-[0_0_15px_rgba(139,92,246,0.8)] border border-violet-400 z-10 animate-pulse">
                            Current Role
                          </div>
                        )}
                        <div className="flex flex-col gap-2 mb-4">
                          ...
                          <span className="text-violet-400 font-medium bg-violet-500/10 self-start md:self-end px-3 py-1 rounded-full text-sm flex items-center gap-2">
                            <Calendar size={14} /> {exp.period}
                          </span>
                        </div>
                        <div className="flex items-center justify-start md:justify-end gap-2 text-slate-400 mb-6 text-sm">
                          <MapPin size={16} className="text-fuchsia-400 md:hidden" />
                          <span>{exp.company} • {exp.location}</span>
                          <MapPin size={16} className="text-fuchsia-400 hidden md:block" />
                        </div>
                  ...
                  {/* Center Node */}
                  <div className="w-10 h-10 absolute left-6 md:left-1/2 transform -translate-x-1/2 flex items-center justify-center z-20">
                    <motion.div
                  ...
                  {/* Right Column */}
                  <div className={`w-full md:w-5/12 ${!isEven ? 'md:pl-8 text-left order-2 pl-16 md:pl-0 mt-8 md:mt-0' : 'hidden md:block opacity-0'}`}>
                    {!isEven && (
                      <motion.div
                        initial={{ opacity: 0, x: 50, scale: 0.8 }}
                        whileInView={{ opacity: 1, x: 0, scale: 1 }}
